...
?>
<div class="report">Generating an input form to match the columns in the table <?php echo $mytable ?> . In script b_form.php</div>
<?php
...
<?php
if(!$valid){echo "<h2>Errors in form - message from server</h2>";}
...
if($webdata['action'] == 'delete'){
	?>
	<div class="report">Action to delete has been confirmed.In script b_form.php</div>
...
elseif($webdata['action'] == 'edit'){
				...
				?>
					<div class="report">Action to edit has been confirmed.In script b_form.php</div>
					 <h2>Edit an existing data record</h2>
					<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"  name = "zy620265delete<?php echo $row["id"]; ?>" >
					<input type="hidden" name= "editid" id= "editid" value="<?php echo $row["id"]; ?>" >
					<input type="hidden" name= "action" id= "action" value="delete" >
					<p><input type="submit" name="delete" id="delete" value="Delete Record:  <?php echo $row["id"] . "  " .htmlspecialchars($row["firstname"]). "  " . htmlspecialchars($row["lastname"]) ;?>"></p>
					</form>
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" onsubmit = "return validate()" name = "zy620265<?php echo $row["id"]; ?>" >
	...
	<input type="text" name="lastname" id="lastname" placeholder="lastname" required minlength="1"  pattern="[^~\|]+" title="Characters ~ and | are restriced" value="<?php if ($valid){echo htmlspecialchars($row["lastname"]);}
	else{echo htmlspecialchars($webdata["lastname"]);} ?>" onchange="myFunction(this.id , this.value, 'reg')">
	...
	<input type="text" name="comments" id="comments" placeholder="comments"  maxlength="65000" minlength="0" value="<?php if ($valid){echo htmlspecialchars($row["comments"]); }else{echo htmlspecialchars($webdata["comments"]);}?>" onchange="myFunction(this.id , this.value, 'reg')">
	<span id="fbcomments">  </span><?php if (!$valid){echo  $formerror["comments"]  ;} ?></p>
	<p>
